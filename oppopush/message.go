...
type Notification struct {
	...
	Category            string      `json:"category,omitempty"`              // 通道类别名，详见《新消息分类场景说明》
	NotifyLevel         int         `json:"notify_level,omitempty"`          // 通知栏消息提醒登记取值定义：1-通知栏 2-通知栏+锁屏 16-通知栏+锁屏+横幅+震动+铃声 使用notify_level 参数时，category参数必传
}
